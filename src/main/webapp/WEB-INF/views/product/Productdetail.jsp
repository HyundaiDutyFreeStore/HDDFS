				...
				<section class="pd_area">
					<div class="pd_visual">
						<div class="info_button">
							<div class="por_icons">
								<span>세일</span><span>사은품</span><span>품절임박</span>
							</div>
							...
							<div class="pd_share">
								<a href="javascript://" onclick="" class="wish_popup">
									...
									</button> <span id="cnrAcumCnt">1</span>
								</a>
								<div class="sns_popup">
									<a href="javascript://">
									...
									<div class="sns_link">
										...
										<a href="javascript://" class="blog shareNaver">blog</a> <a
											href="javascript://" class="kakao shareKakao">kakao</a> <a
											href="javascript:showEmlSendFormLayer();" class="email">email</a>
									</div>
								</div>
							</div>
						</div>
							...
							<div class="swiper-container swiper_visual_thumbnail">
								<div class="swiper-wrapper">
									<c:forEach var="img" items="${imglist }">
										...
										<div class="swiper-slide ">
											<img src=<c:out value="${img}"/> alt="" title=""
						...
						<div class="summary_info">
							<div class="tit">
								<p></p>
								<strong><a
									href="https://www.hddfs.com/shop/dm/bran/brand.do?onlnBranCd=642001"><c:out
											value="${product.pbrand }" /></a>
								<c:out value="${product.pname }" /></strong>
							</div>
							<div class="price">
								<span class="upto"> <i><c:out
											value="${product.pdiscount }" /></i>%
								</span>
								<ul class="del">
									<li class="priceInfo"><del>
											$
											<fmt:formatNumber value="${product.pprice}"
												pattern="#,##0.##" />
										</del> <del class="won">
											<fmt:formatNumber value="${product.pprice*1267}"
												pattern="#,#00" />
											원
										</del></li>
									<li class="dispriceInfo"><strong> $<fmt:formatNumber
												value="${product.pprice*(1-(product.pdiscount/100))}"
												pattern="#,##0.##" /></strong>
										<p>
											<fmt:formatNumber
												value="${product.pprice*(1-(product.pdiscount/100))*1267}"
												pattern="#,#00" />
											원
										</p></li>
								</ul>
					...
					<div class="pd_info">
							...
							<ul class="con">
								<li><strong>
										...
										<div class="square_tooltip">
											...
											<div class="tooltip">
												일반적립금은 주문 시 각 브랜드별로 설정 된 한도내에서 바로 사용할 수 있으며, 즉시할인적립금은 결제금액
												기준으로 '즉시할인' 혹은 '적립' 중 선택하여 사용 가능합니다. * 아래 표기 된 사용률은 당사 및
												브랜드사의 정책에 따라 브랜드별로 상이할 수 있습니다.
												<button class="close">닫기</button>
											</div>
										</div>
								</strong>

									<div class="box">일반적립금 30%</div></li>
								<li><strong>
										<p>H.Point</p>
										<div class="square_tooltip">
											<a href="javascript://" class="btn">도움말</a>
											<div class="tooltip">
												H.Point는 H.Point로 적립되며, 비 H.Point고객은 적립이 불가합니다. H.Point가입 후
												이용 부탁 드립니다. (H.Point 구매적립 대상은 H.Point 통합회원 중 한국 국적 고객입니다.)
												<button class="close">닫기</button>
											</div>
										</div>
								</strong>
									<div class="box">결제금액 0.2 % 적립</div></li>
							</ul>
						</div>

						<div class="productdetail_cart">
							<div class="selectlist">
								<!-- HDDFS 리뉴얼 프로젝트 [이민우-2021.03.22] - 구매레이어 추가 -->
								<div class="selectitem" data-id="10073250016301">
									<strong>${product.pname }</strong>
									<div class="box detailBox">
										<div class="num_amount">
											<input value="-" count_range="m" type="button" class="minus"
												onclick="javascript:fn_qtySubtraction(this);"> <input
												type="number" class="counts" id="0" value="1" min="1"
												max="1091" onkeydown="return checkNumber(event);"
												onfocusout="return changeGoosList(this,event,'0','M','N');"
												onpaste="return false;"> <input value="+"
												count_range="p" type="button" class="plus"
												onclick="javascript:fn_qtyAdd(this);">
										</div>

										<dl class="priceInfo">
											<dt class="dorPrice0"
												value=<c:out value="${product.pprice*(1-(product.pdiscount/100))}"/>>
												$
												<fmt:formatNumber
													value="${product.pprice*(1-(product.pdiscount/100))}"
													pattern="#,##0.##" />
											...
											<dd class="wonPrice0"
												value=<c:out value="${product.pprice*(1-(product.pdiscount/100))*1267}"/>>
												<fmt:formatNumber
													value="${product.pprice*(1-(product.pdiscount/100))*1267}"
													pattern="#,#00" />
												<em>원</em>
								...
								<input type="hidden" id="isTotal" value="Y"> <input
									type="hidden" id="totalCnt" value="1"> <input
									type="hidden" id="totalDorSum" value="148"> <input
									type="hidden" id="totalWonSum" value="183,638"> <input
									type="hidden" id="priceUnit" value="원">
								<script>
									...
									function fn_qtySubtraction(el) {
										...
										if (count - 1 < 1) {
											...
											var dorprice = $(el).closest(
													".detailBox").find(
													".priceInfo").children()
													.eq(0).attr("value");
											var wonprice = $(el).closest(
													".detailBox").find(
													".priceInfo").children()
													.eq(1).attr("value");

											var chDorprice = count * dorprice;
											var chWonprice = "${product.pprice*1267}"
													* count;
											...
											$(el)
													...
													.eq(1)
													.text(
															priceComma(chWonprice
																	.toFixed(0))
																	+ '원');
											$(el).closest(".detailBox").find(
													".priceInfo").children()
													.eq(0)
													.val(chDorprice.toFixed(2));
											$(el).closest(".detailBox").find(
													".priceInfo").children()
													.eq(1)
													.val(chWonprice.toFixed(0));
									...
									function fn_qtyAdd(el) {
												...
												.text(
														'$'
																+ chDorprice
																		...
																		.toLocaleString());
										$(el).closest(".detailBox").find(
												".priceInfo").children().eq(1)
												.text(
														priceComma(chWonprice
																.toFixed(0))
																+ '원');
										$(el).closest(".detailBox").find(
												".priceInfo").children().eq(0)
												.val(chDorprice.toFixed(2));
										$(el).closest(".detailBox").find(
												".priceInfo").children().eq(1)
												.val(chWonprice.toFixed(0));
										...
										$(".totalamount").find(".priceInfo")
												.children().eq(0).val(
														chDorprice.toFixed(2));
										$(".totalamount").find(".priceInfo")
												.children().eq(1).val(
														chWonprice.toFixed(0));
							...
							<div class="totalamount">
								<div>
									총 <strong id="totalamount">1</strong> 개
								</div>
								<dl class="priceInfo">
									<dt>
										$
							...
							<c:choose>
								<c:when test="${product.pstock ne 0}">
									<div class="square_tooltip maxprice">
										<button class="btnxl_type1 btnico btn_tooltip btn_view_rv"
											onclick="javascript:calcMaxDcPrc()" disabled="disabled">
						...
						<ul class="layercall" onclick="preventClick(event);">
							<li class="call_panel_b"><a href="#"
								onclick="preventClick(event);"><p>사은품</p></a></li>
							<li class="call_panel_c"><a href="#"
								onclick="preventClick(event);"><p>상품정보</p></a></li>
							<li class="call_panel_d"><a href="javascript://" onclick="">
									<p>추천상품</p>
							</a></li>
							<li class="call_panel_e"><a href="#"
								onclick="preventClick(event);"><p>상품평</p></a></li>
						</ul>
					</div>
				</section>
			</article>
...
ㆍ미주/호주행 출국 고객께서는 액체류, 젤류를 포함한 구매하신 모든 상품을 인도장에서 직접 수령하시기 바랍니다.</span></div>
	...
	<section class="setproduct">
		<div class="box setGoosEvtList"></div>
		<div class="selectedproduct selectEvtProductList addtocart">
			<div class="zone">
				...
				<a href="javascript:" onclick="cartConsist();" class="addcart_btn">장바구니
					담기</a>
			</div>
			<input type="hidden" name="targetListTotPage" id="targetListTotPage" />
			<input type="hidden" name="layerSetGoosId" id="layerSetGoosId" /> <input
				type="hidden" name="layerGoosChoiClsCd" id="layerGoosChoiClsCd" />
		</div>
	</section>
	...
	<div class="layer_unit pop_o1" style="display: none;">
		<div class="layer_popup">
			<div class="layer_cont">
				...
				<p class="text_ty2">
					이미 장바구니에 담긴 세트입니다.<br> 신규 세트를 구성하거나, 기존 세트에 <br>수량을 추가 할
					수 있습니다.
				</p>

				<ul class="btn_group">
					<li><a href="javascript:evtGoosAddCart('I');"
						class="btnde_type1 mid bg_black">새로담기</a></li>
					<li><a href="javascript:evtGoosAddCart('U');"
						class="btnde_type1 mid">기존 세트 수량 추가하기</a></li>
				</ul>
			</div>
		</div>
	...
	<div class="layer_unit pop_o2" style="display: none;">
		<div class="layer_popup">
			<div class="layer_cont">
				<p class="txt_ty1">중복 세트 안내</p>
				<p class="text_ty2">
					이미 장바구니에 담긴 세트입니다.<br> 중복 세트를 새로 담으시겠습니까?
				</p>
				<ul class="btn_group">
					<li><a href="javascript:evtGoosAddCart('I');"
						class="btnde_type1 mid bg_black">새로담기</a></li>
	...
	<div class="ordertime_btn"
		onclick="location.href='https://www.hddfs.com/shop/om/consmGuide/useNotice.do#orderTimeInfo'"></div>
