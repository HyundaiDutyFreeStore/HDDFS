...
public class PedestrianController {
	...
	public String getConf(HttpServletRequest request) {
		 ...
		 * path_in = path + "\\loginside.csv"; String path_out = path +
		 * "\\logoutside.csv"; // 파일 경로에서 VO 저장된 리스트를 리턴 List<PedestrianVO> data =
		 * pedestrianservice.readCsv(path_in); Iterator<PedestrianVO> it =
		 * data.iterator(); int inside = 0; int outside = 0; while (it.hasNext()) {
		 * it.next(); System.out.println(inside++); } data =
		 * pedestrianservice.readCsv(path_out); it = data.iterator(); while
		 * (it.hasNext()) { it.next(); System.out.println(outside++); } int count =
		 * inside - outside; count = Math.max(count, 0); log.info("inside :" + inside +
