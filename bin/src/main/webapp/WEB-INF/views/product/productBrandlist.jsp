								...
								<div class="color_more_wrap">
									<c:forEach items="${colorMap[status.index] }" var="colorList"
										...
										<a href="javascript:void(0)" class="cl wt"
											...
											cimage1="${colorList.cimage1}"
											cimage2="${colorList.cimage2}"></a>
...
<div class="paging" style="display: block;">
	<c:if test="${pageMaker.prev}">
		<!-- 이전 버튼 -->
			<a href="/listBrand?bname=${bname}&pageNum=${pageMaker.startPage - 1}" class="prev">Previous</a>
	</c:if>
	...
	<c:forEach var="num" begin="${pageMaker.startPage}" end="${pageMaker.endPage}">
			<c:if test="${(pageMaker.startPage+i) <= pageMaker.endPage}">
				<a href="/listBrand?bname=${bname}&pageNum=${num}" class="pageBtn">${num}</a>
			</c:if>      
		</c:forEach>
	</span>  
	<c:if test="${pageMaker.next}">
		<!-- 다음 버튼 -->
			<a href="/listBrand?bname=${bname}&pageNum=${pageMaker.endPage +1}" class="next">Next</a>
	</c:if>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$(".cl").on("click", function(e){
				...
				$(this).closest("li").find("img:eq(0)").attr("src", $(this).attr("cimage1"));
				$(this).closest("li").find("img:eq(1)").attr("src", $(this).attr("cimage2"));
