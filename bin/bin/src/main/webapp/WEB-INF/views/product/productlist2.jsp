	...
	<div id="bodyWrap" class="products">
      ...
      <h3 class="cnts_title ou1804">
         ...
         <img
            ...
            alt="location arr"> 
         <a href="/list?clarge=${category.clarge}&cmedium=${category.cmedium}&csmall="
            >${category.cmedium}</a>
...
<div class="paging" style="display: block;">
	...
	<c:if test="${pageMaker.prev}">
		<!-- 이전 버튼 -->
			<a class="prev" href="/list?clarge=${category.clarge}&cmedium=${category.cmedium}&csmall=${category.csmall}&pageNum=${pageMaker.startPage - 1}">Previous</a>
	</c:if>
	...
	<c:forEach var="num" begin="${pageMaker.startPage}" end="${pageMaker.endPage}">
			<c:if test="${(pageMaker.startPage+i) <= pageMaker.endPage}">
				<a href="/list?clarge=${category.clarge}&cmedium=${category.cmedium}&csmall=${category.csmall}&pageNum=${num}" class="pageBtn">${num}</a>
			</c:if>      
	</c:forEach>
	</span>  
	<c:if test="${pageMaker.next}">
		<!-- 다음 버튼 -->
			<a href="/list?clarge=${category.clarge}&cmedium=${category.cmedium}&csmall=${category.csmall}&pageNum=${pageMaker.endPage +1}" class="next">Next</a>
	</c:if>
		...
		$(document).ready(function(){
			...
			$(".cl").on("click", function(e){
				...
				// 김나형 상품디테일 선택한 색으로 변경
 				$(this).closest("li").find("a:eq(0)").attr("href", "/product/productDetail?pid="+$(this).attr("productid")+"&ccolorcode="+$(this).attr("ccolorcode"));
			});
